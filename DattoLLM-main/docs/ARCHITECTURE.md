...
|---|---|---|---|---|
| `apisix` | `/api/auth/*` | GET, POST | No (rate-limited) | Proxied to auth-service |
| `apisix` | `/api/chat` | POST | JWT (Bearer) | Proxied to ai-service; injects `X-User-Id`, `X-User-Role`, `X-Allowed-Tools` headers |
| `apisix` | `/api/chat/mode` | POST | JWT | Proxied to ai-service; toggle cached/live mode |
| `apisix` | `/api/history` | GET | JWT | Proxied to ai-service |
| `apisix` | `/api/history/*` | GET | JWT | Proxied to ai-service |
| `apisix` | `/api/admin/*` | GET, POST, PUT, PATCH, DELETE | JWT | Proxied to ai-service |
| `apisix` | `/api/debug/*` | GET | JWT | Proxied to ai-service |
| `apisix` | `/api/tools` | GET | JWT | Proxied to ai-service |
| `apisix` | `/api/approvals/*` | GET, POST | JWT | Proxied to ai-service |
| `apisix` | `/api/proposals/*` | GET, POST | JWT | Proxied to ai-service |
| `apisix` | `/chat` | POST | JWT | SSE streaming chat — proxied to ai-service |
| `apisix` | `/*` | any | No | Proxied to web-app (catch-all) |

> **Routes are not stored in the repo.** They are pushed to APISIX's etcd-backed Admin API via `./setup-apisix.sh` and must be re-applied after every fresh `docker compose up`.
...
| `JWT_PRIVATE_KEY` | `auth-service` | `.env` file — RS256 PEM, **base64-encoded** (entire PEM file base64'd to one line; NOT raw PEM with `\n` literals) |
| `JWT_PUBLIC_KEY` | `auth-service` | `.env` file — RS256 PEM, **base64-encoded**; also read by `setup-apisix.sh` to configure the APISIX JWT consumer |
| `ANTHROPIC_API_KEY` | `litellm` | `.env` file — only needed for direct-Anthropic routes in LiteLLM config |
| `OPENROUTER_API_KEY` | `litellm`, `ai-service` | `.env` file — primary LLM routing key; routes all Claude/DeepSeek/Gemini via OpenRouter |
| `LITELLM_URL` | `ai-service` | `docker-compose.yml` → `http://litellm:4000` (absent = direct OpenRouter fallback) |
| `LITELLM_MASTER_KEY` | `litellm` | `.env` file — **must start with `sk-`** (e.g. `sk-litellm-admin`). LiteLLM rejects other formats. |
...
Critical facts discovered during real deployment. Must follow exactly on every fresh deploy.

**PgBouncer password sync:**
`services/pgbouncer/pgbouncer.ini` has a hardcoded `password=` in the `[databases]` section. This must match `POSTGRES_PASSWORD` in `.env`. On a fresh deploy with a randomly-generated password, pgbouncer will fail to connect to PostgreSQL until `pgbouncer.ini` is updated:
```ini
[databases]
datto_rmm = host=postgres port=5432 dbname=datto_rmm user=postgres password=<POSTGRES_PASSWORD>
litellm   = host=postgres port=5432 dbname=litellm   user=postgres password=<POSTGRES_PASSWORD>
```
After updating, bring pgbouncer back up with `docker compose up -d pgbouncer` (not `restart`).

**JWT key format:**
`JWT_PRIVATE_KEY` and `JWT_PUBLIC_KEY` must be stored as **base64-encoded PEM** in `.env`. The entire PEM file is base64'd to a single line. `tokens.ts` checks: if the value starts with `-----` it is treated as raw PEM; otherwise it is base64-decoded. Raw PEM with literal `\n` (e.g. from `awk printf "%s\\n"`) does NOT work.
```bash
base64 -w 0 private.pem   # Linux
base64 -i private.pem     # macOS
```

**APISIX consumer `private_key` field:**
When `setup-apisix.sh` creates the JWT consumer, the payload includes `"private_key": ""`. This empty field is required — omitting it causes APISIX Admin API to return HTTP 400.

**LITELLM_MASTER_KEY format:**
LiteLLM rejects master keys that do not start with `sk-`. Use `sk-litellm-admin` or `sk-<random-hex>`.

**APISIX routes are not persisted in the repo:**
Routes, upstreams, and the JWT consumer are stored in etcd (not in any file). After every fresh `docker compose up`, run:
```bash
./setup-apisix.sh
```
Re-running is safe (all calls are PUT — idempotent).

**Use `up -d`, not `restart`, after `.env` changes:**
`docker compose restart <service>` does NOT reload environment variables. Always use `docker compose up -d <service>`.
...
### 14.15 CVE Vulnerability Scanner

A self-contained Docker service that downloads NVD CVE data, indexes it locally, and matches against the device software inventory to find known vulnerabilities.

**Container:** `cve-scanner` (port 8500, internal only)
**Source:** `cve-scanner/` — separate from all other services
**Database:** `cve_database` (194K CVEs), `cpe_dictionary` (1M+ CPE entries), `device_vulnerabilities` (matches), `cve_sync_log`

**Architecture:**
```
FKIE NVD JSON Feeds (GitHub) → cve-scanner container
  → Download .json.xz → decompress → parse
  → Index CVEs + CPE entries into PostgreSQL
  → Match against datto_cache_device_software
    → PRODUCT_MAP direct mapping (95% confidence)
    → Vendor extraction + fuzzy matching (90%)
    → Version range filtering (endExcluding/endIncluding)
  → Store results in device_vulnerabilities
  → Dashboard at /admin/explorer/vulnerabilities
```

**Matching algorithm (3 tiers):**
1. **Direct mapping** (confidence 0.95): `PRODUCT_MAP` maps software names to CPE vendor:product (e.g., "Google Chrome" → `google:chrome`)
2. **Vendor extraction** (confidence 0.90): Extract vendor from first word, fuzzy match remaining against CPE products
3. **Alt-product search**: Adobe (`acrobat` ↔ `acrobat_dc`), Foxit (`foxit_reader` ↔ `pdf_reader` ↔ `phantompdf`), Dell (`supportassist_for_business_pcs` ↔ `supportassist_for_home_pcs`)

**Version filtering:**
- Only matches CVEs that have version range data in CPE (no wildcards)
- Compares installed version against `versionStartIncluding`/`versionEndExcluding`/`versionEndIncluding`
- Version scheme ratio check: skips if major versions differ by 3x+ (handles Adobe 25.x vs 120.x)
- Wildcard product blocklist: M365 Apps, Microsoft Office (NVD uses `-` for version = matches everything)

**Validation results (5 rounds, 90+ entries tested):**

| Tier | Products | Accuracy |
|---|---|---|
| EXACT (95%+) | 50 | 99.7% |
| GOOD (80-95%) | 12 | 90.7% |
| Overall | 62 | 97.7% |

**API endpoints (in ai-service):**

| Endpoint | Purpose |
|---|---|
| `GET /api/admin/browser/vulnerabilities/summary` | Severity counts, top software, top sites, last scan |
| `GET /api/admin/browser/vulnerabilities` | Paginated list with filters (severity, site, device, software) |
| `GET /api/admin/browser/devices/:uid/vulnerabilities` | CVEs for a specific device |
| `POST /api/admin/cve-scan` | Trigger manual scan |
| `GET /api/admin/cve-scan/status` | Scan status + CVE/match counts |

**Schedule:** Daily at 4 AM (configurable via `SCAN_CRON`). Current year's NVD feed always re-downloaded; older years cached.

**Known limitations:**
- M365/Office excluded (NVD wildcard CPEs)
- NVD CPE data lag for very recent CVEs (24-48h)
- Adobe older-track CPEs (17.x/20.x) don't match installed 25.x versions (different numbering scheme)
...
### New Files (v2.5.0 — CVE Scanner + Voice Gateway + Data Improvements)

| File | Purpose |
|---|---|
| `cve-scanner/` | Self-contained CVE vulnerability scanner service — downloads NVD feeds, indexes 194K CVEs + 1M CPE entries, matches against device software with version range filtering. 99.7% accuracy on top 50 products. |
| `cve-scanner/src/index.ts` | Express server + cron scheduler + scan pipeline orchestration |
| `cve-scanner/src/nvdFetcher.ts` | Download NVD JSON feeds from FKIE GitHub (`.json.xz` compressed) |
| `cve-scanner/src/cveIndexer.ts` | Parse CVE JSON → upsert into `cve_database` + `cpe_dictionary` |
| `cve-scanner/src/matcher.ts` | Fuzzy match device software against CPE dictionary with version range filtering |
| `ai-service/src/vulnBrowser.ts` | Admin API endpoints: vulnerability summary, list, device vulns, CVE scan trigger |
| `services/web-app/src/app/admin/explorer/vulnerabilities/page.tsx` | Vulnerability dashboard — severity cards, top software/sites, paginated CVE list |
| `db/021_data_improvements.sql` | New device columns (domain, last_user, reboot), views (v_site_health, v_device_health, v_os_distribution, etc.) |
| `db/022_cve_scanner.sql` | CVE scanner tables: `cve_database`, `cpe_dictionary`, `device_vulnerabilities`, `cve_sync_log` + views |
| `services/asterisk/` | Self-hosted Asterisk PBX config (replaces 3CX) for voice gateway |
| `services/freeswitch/mrf.xml` | Custom FreeSWITCH SIP profile with `ext-rtp-ip` templating |
