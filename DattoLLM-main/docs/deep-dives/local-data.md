...
tags:
  ...
  - sync
type: Deep Dive
description: Local Datto data cache architecture — sync pipeline, cached queries, fuzzy search, and permission gates
aliases:
  - Local Data Cache
  - Datto Cache
---
...
> Parent: [[PLATFORM_BRAIN]] · See also: Local Data Cache section in PLATFORM_BRAIN
> Status: **Implemented** — sync pipeline, cached queries, fuzzy search, and SEC-Cache-001 permission gate all operational · Design history: [[FUZZY-SEARCH-PLAN]]
> Purpose: Pull Datto RMM data into local [[PostgreSQL]] on a schedule. [[AI Service]] queries local DB instead of calling Datto API live. A "Live" button bypasses the cache for real-time data.
...
LIVE MODE (on demand — user clicks "Live" button)
  AI calls Datto API via MCP Bridge in real time → current behaviour, always fresh
```

> [!warning] Cached data is stale by design
> Use live mode when you need current data (e.g. checking if an alert was just resolved). Alert data can be up to 59 minutes old in cached mode — see [[SECURITY_FINDINGS|SEC-012]].
...
> [!info] Rate limit budget
> Datto API rate limit is 600 requests per 60 seconds. The sync pipeline enforces a 480-request cap to leave headroom for live queries. See [[MCP Server to Datto]] for rate limit details.
...
Instead of tool results coming from the Datto API, they come from SQL queries on the cache tables. The [[AI Service]] receives the same JSON format — it cannot tell the difference (see [[Chat Request Flow]] for the full pipeline). Each cached response includes a note:
...
> When `dataMode === "cached"`, tool calls bypass the [[MCP Bridge]] entirely — executing directly against `datto_cache_*` tables via `executeCachedTool()`. Layer 1.5 (`permissions.ts`) is the **only hard gate** for cached tools. See [[SEC-CACHE-001-PLAN]] for the full design and [[SECURITY_FINDINGS]] for status.
...
[[AI Service]] · [[PostgreSQL]] · [[MCP Bridge]] · [[MCP Server]] · [[RBAC System]] · [[Data Explorer]] · [[FUZZY-SEARCH-PLAN]] · [[SEC-CACHE-001-PLAN]] · [[SECURITY_FINDINGS]] · [[Chat Request Flow]]
