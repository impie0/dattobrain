...
This project is indexed by GitNexus as **datto** (851 symbols, 1682 relationships, 57 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
This project includes an Obsidian vault (`.obsidian/`) with knowledge graph nodes. Use these skills when working with vault files.

## When to use each skill

| Task | Skill file |
|------|-----------|
| Read/create/search notes, manage tasks, interact with a running Obsidian instance | `.agents/skills/obsidian-cli/SKILL.md` |
| Write `.md` files with wikilinks, callouts, embeds, frontmatter, tags | `.agents/skills/obsidian-markdown/SKILL.md` |
| Create or edit `.canvas` files (visual maps, mind maps, flowcharts) | `.agents/skills/json-canvas/SKILL.md` |
| Create or edit `.base` files (database views, table/card/list views, formulas) | `.agents/skills/obsidian-bases/SKILL.md` |
| Fetch and clean web content into Obsidian-ready markdown | `.agents/skills/defuddle/SKILL.md` |

## Quick rules

- Use `[[wikilinks]]` for internal vault links — never Markdown links for notes inside the vault
- All `.md` node files in the vault (e.g. `AI Service.md`, `ARCHITECTURE.md`) follow Obsidian Flavored Markdown — load `obsidian-markdown` skill before editing them
- The vault graph nodes live alongside the project source; keep them in sync with code changes (see "Keep the docs current" rules above)
- Use `obsidian-cli` skill for vault operations only when Obsidian is running; use file tools (Read/Edit/Write) otherwise
